...
public class AsyncDatastoreImpl implements AsyncDatastore {
	...
	private <T extends Object> void create(T object, boolean insert) {
		...
		for (Method method : DynamoDBReflector.INSTANCE
			...
			if (method.equals(hashKeyGetter)) {
				if (getterResult == null
					...
					updateValues.put(attributeName,
							new AttributeValueUpdate().withAction("PUT")
			...
			} else if (DynamoDBReflector.INSTANCE.isVersionAttributeGetter(method)) {
				if (!insert) {
		...
		if (insert || updateValues.isEmpty()) {
			...
			try {
				...
				throw new DuplicateEntityException("Entity already exists with key: " + objectKey.toString());
			}
		} else {
			phoebe.getClient().updateItem(
						...
						DeleteItemRequest deleteItemRequest = new DeleteItemRequest()
								...
								.withTableName(
										entityKey.getDynamoDBTable()
												.tableName());
						phoebe.getClient().deleteItem(deleteItemRequest);
						return null;
					}
				}));
	}
		...
		return new FutureResultAdapter<T>(phoebe.getExecutorService().submit(
				new Callable<T>() {
					public T call() throws Exception {
						...
						return phoebe.getMapper().marshallIntoObject(
							...
							do {
								...
								while (classIterator.hasNext()) {
									...
									for (Map<String, AttributeValue> item : batchResponse
											.getItems()) {
										obj = phoebe.getMapper()
												.marshallIntoObject(kindClass,
														item);
										result.add(obj);
									}
								}
		...
		return new FutureResultAdapter<UpdateResult<T>>(phoebe
				.getExecutorService().submit(new Callable<UpdateResult<T>>() {
					public UpdateResult<T> call() throws Exception {
						...
						try {
							phoebe.getClient().updateItem(updateItemRequest);
